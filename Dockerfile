...
RUN python -m venv /opt/pyordstat-env
ENV PATH /opt/pyordstat-env/bin:$PATH
ENV VIRTUAL_ENV /opt/pyordstat-env

# Set the working directory.
WORKDIR /workspaces/pyordstat/

# Install the run time Python dependencies in the virtual environment.
COPY --chown=user:user poetry.lock* pyproject.toml /workspaces/pyordstat/
RUN mkdir -p /home/user/.cache/pypoetry/ && mkdir -p /home/user/.config/pypoetry/ && \
    mkdir -p src/pyordstat/ && touch src/pyordstat/__init__.py && touch README.md
...
COPY --chown=user:user .pre-commit-config.yaml /workspaces/pyordstat/
